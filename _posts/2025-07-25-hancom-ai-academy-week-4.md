...
- **`React`**: 이번 프로젝트는 **`Next.js`**, **`React`** 중 하나를 선택하여 진행할 수 있었습니다. **`Next.js`** 는 한 번도 사용해본 적 없지만 새로운 기술에 도전할 기회가 생긴다는 장점이 있었고, **`React`** 는 이제껏 강의에서 여러 번 사용하여 꽤나 익숙하며 아직 부족한 숙련도를 다질 수 있다는 장점이 있었는데, 저희 팀은 이 중 **`React`** 를 선택하였습니다.
